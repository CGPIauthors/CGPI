...
class Logger(object):
    def __init__(self):
        ...
        self._snapshot_gap = np.asarray([1])
    ...
    def _add_output(self, file_name, arr, fds, mode='a', open_func=open):
        if file_name not in arr:
            mkdir_p(os.path.dirname(file_name))
            arr.append(file_name)
            fds[file_name] = open_func(file_name, mode)
    ...
    def add_text_output(self, file_name, mode='a', open_func=open):
        self._add_output(file_name, self._text_outputs, self._text_fds,
                         mode=mode, open_func=open_func)

    def remove_text_output(self, file_name):
        self._remove_output(file_name, self._text_outputs, self._text_fds)

    def add_tabular_output(self, file_name, relative_to_snapshot_dir=False, mode='w', open_func=open):
    ...
    def set_snapshot_gap(self, gap):
        try:
            ...
            gap = [gap]
        self._snapshot_gap = np.asarray(gap)
    ...
    def set_no_stdout(self, no_stdout):
        self._no_stdout = no_stdout

    def get_no_stdout(self):
        return self._no_stdout
    ...
    def dump_tabular(self, *args, **kwargs):
        ...
        flush = kwargs.pop("flush_file", True)
        skip_text_logging = kwargs.pop("skip_text_logging", True)
        if len(self._tabular) > 0:
            if self._log_tabular_only or skip_text_logging:
                pass
    ...
    def save_itr_params(self, itr, get_params):
        itr += 1
        if self._snapshot_dir:
            if self._snapshot_mode == 'all':
                file_name = osp.join(self._snapshot_dir, 'itr_%0.6d.pkl' % itr)
                torch.save(get_params(), file_name)
            elif self._snapshot_mode == 'last':
                # override previous params
                file_name = osp.join(self._snapshot_dir, 'params.pkl')
                torch.save(get_params(), file_name)
            elif self._snapshot_mode == "gap":
                if np.any(itr % self._snapshot_gap == 0):
                    file_name = osp.join(self._snapshot_dir, 'itr_%0.6d.pkl' % itr)
                    torch.save(get_params(), file_name)
